                ...
                tuning=c("none","aic","bic","cv"),folds=5)
{
    ...
    num_iter = 0
    # diff = .C("dpm", as.double(S), diff=double(d*nlambda), as.integer(d), as.integer(diags),as.double(lambda),
    #         as.integer(nlambda), as.double(gamma), as.integer(max.ite),
    #         as.double(prec), as.double(e), num_iter = as.double(num_iter))$diff;
    result = .C("dpm", as.double(S), diff=double(d*nlambda), as.integer(d), as.integer(diags),as.double(lambda),
              as.integer(nlambda), as.double(gamma), as.integer(max.ite),
              as.double(prec), as.double(e), num_iter = as.double(num_iter));
    diff = result$diff;
    ...
    return(list(path=ret,lambdas=lambda,nlambda=nlambda,opt=opt, Sigma.X = S0, Sigma.Y = S1, n.X = n1, n.Y=n0, iter=num_iter));
}
