assymmetric_apg = function(X, Y, lambdas=NULL, lambda.min.ratio = NULL, nlambda = NULL, a = NULL,
    ...
    for(i in 1:length(lambdas)){
        ...
        out = .C("assymmetric_lasso", as.double(fit$Sigma.X), as.double(fit$Sigma.Y), as.integer(p),
        ...
        if(method[1] == "scad"){
            out = .C("assymmetric_scad", as.double(fit$Sigma.X), as.double(fit$Sigma.Y), as.integer(p),
        ...
        if(method[1] == "mcp"){
            out = .C("assymmetric_mcp", as.double(fit$Sigma.X), as.double(fit$Sigma.Y), as.integer(p),
                     as.double(lambda), as.double(a), as.double(X), as.double(Y), as.integer(n.X), as.integer(n.Y),
                     as.double(epsilon.X), as.double(epsilon.Y),
                     as.double(lip), as.double(stop.tol), as.integer(max.iter), as.integer(verbose),
                     Delta = as.double(Delta))
